...
- **Web UI** at `192.168.4.1` (AP mode, SSID "POV-Display") — patterns, color, brightness, phase, refresh rate, motor start/stop toggle, preferences reset, and controller reboot. Motor speed is closed-loop: refresh rate and `NUM_ARMS` define the RPM target, `HW_MAX_RPM` caps it, and EMA-filtered Hall feedback adjusts the ESC pulse via proportional correction; `freshHallRpm()` predicts deceleration from elapsed time between triggers and falls back to bounded startup ramp only after the stale timeout. No reflash needed.
...
- **Logging** uses ESP-IDF `ESP_LOGx` macros via `src/log_tags.h` (`POV_LOGx` wrappers). Each source file declares `LOG_TAG(name);` and uses `POV_LOGI`, `POV_LOGD`, etc. Runtime log level (Error/Warning/Info/Debug) is a setting in the Hardware tab, persisted to NVS, and applied globally via `esp_log_level_set`. Build-time tag exclusion: add `-DLOG_EXCLUDE_TAGS='"scanner","spi"'` to `platformio.ini` build flags. Config changes and hardware readings use Info; verbose diagnostics (pixel dumps, idle Hall, rotation metrics, SPI sends) use Debug.
...
- `docs/concepts/` — short explainers on EE/ME/physics concepts: blade aerodynamics, closed-loop speed control, ESC arming & calibration, fisheye distortion, power budget & runtime estimates, perception rendering, polar distortion correction.
