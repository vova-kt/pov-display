...
- **Serial diagnostics**: boot, resize, Hall, and motor-control logs stay enabled; periodic `[hall]` logs lead with expected/actual/error RPM, while verbose `[dbg]` pixel/frame and `[hall-dbg]` idle-Hall logs remain in `src/main.cpp` but are disabled by default.
